...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_output = random.choice(language_prompts["WELCOME"]).format(language_prompts["SKILL_NAME"])
...
#This Handler takes the slot value passed by the user and saves it as a session attribute.
class MyNameIsIntentHandler(AbstractRequestHandler):
    ...
    def handle(self,handler_input):
        ...
        speech_output = random.choice(language_prompts["NAME_RESPONSE"]).format(user_name)
        reprompt = random.choice(language_prompts["NAME_RESPONSE_REPROMPT"])
        
        handler_input.attributes_manager.session_attributes["user_name"] = user_name
...
#This Handler fetches the users name from the session attribute and speaks it back.
class SayNameIntentHandler(AbstractRequestHandler):
    def can_handle(self, handler_input):
    ...
    def handle(self,handler_input):
        ...
        if user_name:
            speech_output = random.choice(language_prompts["SAY_NAME_RESPONSE"]).format(user_name)
        else:
            speech_output = random.choice(language_prompts["NO_NAME_RESPONSE"])
...
sb.add_request_handler(SayNameIntentHandler())
